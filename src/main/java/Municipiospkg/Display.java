...
public class Display extends JFrame {
  ...
  /**
   * Iniciar la aplicacion con el main
   */
  ...
  public static Dijkstra readGraph(String vertexFile, String edgeFile) { //leer grafos

    Dijkstra dijkstra = new Dijkstra();//se instancia la clase dijkstra
    try {
      String line;
      // Leer los vertices
      ...
      while ((line = vertexFileBr.readLine()) != null) {
        ...
        if (parts.length != 3) {
          vertexFileBr.close();
          throw new IOException("Linea no valida en el archivo " + line);
        }
      ...
      while ((line = edgeFileBr.readLine()) != null) {
        ...
        if (parts.length != 3) {
          edgeFileBr.close();
          throw new IOException("Linea invalida en el archivo " + line);//por si da error a la hora de leer el archivo txt
        }
        dijkstra.addUndirectedEdge(parts[0], parts[1], Double.parseDouble(parts[2]));
      }
      edgeFileBr.close();
    } catch (IOException e) {
      System.err.println("No se pudo leer el Dijkstra" + e);
      return null;
    }
    return dijkstra;
  }
  ...
  /**
   * Crear o mostrar la ventana con las opciones
   */
  public Display() {
    setTitle("Visualizador de Dijkstra");
    ...
    panel = new GraphPanel(dijkstra);//instanciamos lo de panel
    ...
    chckbxShowEdgeWeights = new JCheckBox("Mostrar los pesos");
    ...
    JLabel lblReloadGraph = new JLabel("Cargar / Recarga Dijkstra");
    GridBagConstraints gbc_lblReloadGraph = new GridBagConstraints();//para recargar
    ...
    txtCityxytxt.setText("cityxy.txt"); //archivo txt con los municipios
    ...
    JButton btnReloadGraph = new JButton("Cargar / Resetear");
    GridBagConstraints gbc_btnReloadGraph = new GridBagConstraints(); //resetear
    ...
    contentPane.add(btnReloadGraph, gbc_btnReloadGraph); //cargar
    ...
    JLabel lblEuclideanCosts = new JLabel("Mostrar Costos"); //para los botones
    ...
    JButton btnComputeAllEuclidean = new JButton("Compara las distancias");//distancias comparadas
    ...
    JLabel lblGetWeightedShortest_1 = new JLabel("Algoritmo de Dijkstra"); //mensaje con sus dimensiones
    ...
    JLabel lblStart_1 = new JLabel("Inicia");
    ...
    weightedStartCityComboBox = new JComboBox<>();//seguir dibujando la ventana
    ...
    JLabel lblEnd_1 = new JLabel("Final");
    ...
    JButton btnDrawWeightedShortest = new JButton("Dibuja el camino de Dijkstra"); //para dibujar el camino de la ruta mas corta
    ...
    JLabel lblPanda = new JLabel("Programa para calcular dijkstra>");//mensaje de hasta abajo
    lblPanda.setForeground(UIManager.getColor("TextField.light"));
    lblPanda.addMouseListener(new MouseAdapter() {
      //quite un mensaje del menu
    });
    ...
    btnDrawWeightedShortest.addMouseListener(new MouseAdapter() {
      ...
      public void mouseReleased(MouseEvent e) {
        ...
        System.out.println("Calculo de ruta mas corta para " + startCity + " hacia " + endCity);
  ...
  private void updateGraphPanel() { //refrescar el panel de grafos
    ...
    System.out.println("Construyendo un nuevo archivo " + vertexFile + " y " + edgeFile);
    System.out.println("Leer datos " + panel.dijkstra.getVertices());
  ...
  public class GraphPanel extends JPanel {
    ...
    public void paintComponent(Graphics g) {
      ...
      super.paintComponent(g2); // Mostrar el panel
      try {
        paintGraph(g2); // Dibujar el dijkstra
    ...
    public void paintEdge(Graphics g, Vertex u, Vertex v, double weight, Color color, int thickness, int alpha) {
      ...
      if (chckbxShowEdgeWeights.isSelected()) {
        ...
        g2.setFont(new Font("Helvetica", Font.PLAIN, 8));//fuente de letra
